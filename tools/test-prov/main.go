...
func main() {
	...
	for i = 0; i < *num; i++ {
		...
		if *ssl {
			...
			ca, err := x509.ParseCertificate(caByte)

			if err != nil {
				log.Fatalf("Failed to load CA cert")
			}
			log.Printf("Loaded CA cert %s", ca.Subject.CommonName)
			// 		thing_cert:
			...
			derBytes, err := x509.CreateCertificate(rand.Reader, &tmpl, ca, publicKey(priv), priv)
			if err != nil {
		...
		connections = append(connections, conns{ch.ID, m.ID, m.Key, cert, key})

		cli.LogJSON(connections)
	}
}
